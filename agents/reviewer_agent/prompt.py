...
def _review_format_guidance(policy: ScriptLengthPolicy) -> str:
    return (
        ...
        "When rejecting, report every material blocking or required change observable in the "
        "current candidate; do not intentionally defer known issues to a future review. Keep "
        "optional improvements separate, and never reject solely to pursue an optional warning "
        "or a higher style score. Approve when deterministic gates pass, required_changes is "
        "empty, and no critical or blocking finding remains; perfection is not required. "
